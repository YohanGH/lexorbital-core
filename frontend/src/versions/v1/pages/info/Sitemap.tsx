...
export function Sitemap(): JSX.Element {
  ...
  const handleNavigate = (page: string): void => {
    ...
    if (isVersionedPage) {
      // Use versioned route (compact format: /v1/home)
      try {
        const latestVersion = getLatestVersion() // Already has 'v' prefix (e.g., "v1")
        const targetPath = `/${latestVersion}/${page}`
        setLocation(targetPath)
      } catch (error) {
        console.error("Failed to get latest version:", error)
        // Fallback to a default version
        const targetPath = `/v1/${page}`
        setLocation(targetPath)
      }
    } else {
