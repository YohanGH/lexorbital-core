...
export function RedirectToLatest(): JSX.Element {
  ...
  useEffect(() => {
    const redirect = async (retries = 0): Promise<void> => {
      try {
        const latest = await getLatestVersion()
        setError(null)
        setLocation(`/v/${latest}/home`)
      } catch (err) {
          ...
          <button
            onClick={() => {
              ...
              void getLatestVersion()
                .then(latest => setLocation(`/v/${latest}/home`))
                .catch(() => setError("Erreur lors du rechargement"))
